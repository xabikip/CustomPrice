...
/*Funcion que mueve el tab para que la personalización
del producto sea la primera
*/
$(document).ready(function () {
	$('a[href="#idTab10"]').parent().prependTo($('#more_info_tabs'));
	$("#more_info_tabs").idTabs("idTab10");
    //Deshabilitamos los select de cantidad de caracteres
	document.getElementById("group_10").disabled=true;
    document.getElementById("group_11").disabled=true;
    // $('select#group_11').hide();
});
...
{
	var valorText = $('textarea#textField0').val();
	var valorNum = $('textarea#textField1').val();
	if (isNaN($('textarea#textField1').val())){
		alert('En el campo de numero solo podra introducir numeros');
	}
	var text_cant =  valorText.length;
	text_cant_str = text_cant.toString();
	var num_cant =  valorNum.length;
	num_cant_str = num_cant.toString();
 
	var text_max = $('span.textMax').html();
	var num_max = $('span.numMax').html();
	text_max = parseInt(text_max);
	num_max = parseInt(num_max);

	
	if (text_cant <= text_max)
	{

		if (num_cant <= num_max)
		{
			$("select#group_10 option:contains(" + text_cant_str + ")").attr('selected', true);
			$("select#group_11 option:contains(" + num_cant_str + ")").attr('selected', 'selected');
			findCombination();
			getProductAttribute();
		...
		{
			alert('La cantidad de numeros introducidos no puede ser mayor que ' + num_max);
		}
		
	}
	else
	{
		alert('La cantidad de caracteres introducidos en el texto no puede ser mayor que ' + text_max);
	}
}
